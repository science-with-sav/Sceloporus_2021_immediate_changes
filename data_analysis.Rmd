...
Now do the exact same thing, but for surface body temperature
```{r}
lizard_tsurf_time_lm_log <- lm(data = temp_time_series, 
                          log(calibrated_dorsal_temp) ~ 
                            log(time_min)*lizard_ID)
plot(lizard_tsurf_time_lm_log)
```
...
```{r}
lizard_coeffs_tsurf <- data.frame(emtrends(lizard_tsurf_time_lm_log, 
                                          "lizard_ID", var = "time_min")) %>%
   left_join(lizard_tmt_info, 
             by = c("lizard_ID" = "individual_ID")) %>%
   dplyr::select(lizard_ID, rate_change = time_min.trend, 
   ...
   dplyr::mutate(date_factor = as.factor(date))
summary(lizard_coeffs_tsurf)
length(unique(lizard_coeffs_tsurf$lizard_ID))
```

then get average rate of change by treatment group:

```{r}
avg_tsurf_change <- lm(data = lizard_coeffs_tsurf, rate_change ~ treatment)
summary(avg_tsurf_change)
avg_tsurf_change_coeff <- data.frame(emmeans(avg_tsurf_change, "treatment"))
avg_tsurf_change_coeff
avg_tsurf_change_pairs <- data.frame(pairs(emmeans(avg_tsurf_change, "treatment")))
avg_tsurf_change_pairs
```
...
```{r}
lizard_coeffs_tsurf_control <- lizard_coeffs_tsurf %>% 
  dplyr::filter(treatment == "Control")
t.test(lizard_coeffs_tsurf_control$rate_change)

lizard_coeffs_tsurf_cool <- lizard_coeffs_tsurf %>% 
  dplyr::filter(treatment == "Cooling")
t.test(lizard_coeffs_tsurf_cool$rate_change)

lizard_coeffs_tsurf_heat <- lizard_coeffs_tsurf %>% 
  dplyr::filter(treatment == "Heating")
t.test(lizard_coeffs_tsurf_heat$rate_change)
```
...
```{r}
lizard_CEWL_tsurf_loglog_lm <- lm(data = temp_time_series, 
                        log(CEWL_g_m2h) ~ log(calibrated_dorsal_temp) * lizard_ID)
plot(lizard_CEWL_tsurf_loglog_lm)
```
...
```{r}
lizard_coeffs_CEWL_tsurf <- data.frame(emtrends(lizard_CEWL_tsurf_loglog_lm, 
                                          "lizard_ID", 
                                          var = "calibrated_dorsal_temp")) %>%
   left_join(lizard_tmt_info, 
             by = c("lizard_ID" = "individual_ID")) %>%
   dplyr::select(lizard_ID, rate_change = calibrated_dorsal_temp.trend, 
                 SE, df, lower.CL, upper.CL,
                 date,
                 treatment
                 ) %>%
   dplyr::mutate(date_factor = as.factor(date))
length(unique(lizard_coeffs_CEWL_tsurf$lizard_ID))
```

then get average rate of change by treatment group:

```{r}
avg_CEWL_tsurf_change <- lm(data = lizard_coeffs_CEWL_tsurf, 
                            rate_change ~ treatment)
summary(avg_CEWL_tsurf_change)
avg_CEWL_tsurf_change_coeff <- data.frame(emmeans(avg_CEWL_tsurf_change, 
                                                  "treatment"))
avg_CEWL_tsurf_change_coeff
avg_CEWL_tsurf_change_pairs <- data.frame(pairs(emmeans(avg_CEWL_tsurf_change,
                                                        "treatment")))
avg_CEWL_tsurf_change_pairs
```
...
```{r}
lizard_coeffs_CEWL_tsurf_control <- lizard_coeffs_CEWL_tsurf %>% 
  dplyr::filter(treatment == "Control")
t.test(lizard_coeffs_CEWL_tsurf_control$rate_change)

lizard_coeffs_CEWL_tsurf_cool <- lizard_coeffs_CEWL_tsurf %>% 
  dplyr::filter(treatment == "Cooling")
t.test(lizard_coeffs_CEWL_tsurf_cool$rate_change)

lizard_coeffs_CEWL_tsurf_heat <- lizard_coeffs_CEWL_tsurf %>% 
  dplyr::filter(treatment == "Heating")
t.test(lizard_coeffs_CEWL_tsurf_heat$rate_change)
```
...
# save
ggsave(filename = "methods_schematic.tiff",
