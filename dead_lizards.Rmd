...
control1  <- c(brewer.pal(9, "Greys")[c(7)])
control2  <- c(brewer.pal(9, "Greys")[c(5)])
```
...
ggplot(CEWL_time_temp_subset) +
  ...
  #geom_smooth(aes(x = (time_min), 
   ...
   annotate("text", label = "Cool", 
            x = 15, y = -8.5, size = 4, color = cool) +
   annotate("text", label = "Control[Cool]", parse = T,
            x = 14.8, y = -2.8, size = 3, color = control1) + 
   
   annotate("text", label = "Heat", 
            x = 15, y = 24, size = 4, color = heat) +
   annotate("text", label = "Control[Heat]", parse = T,
            x = 14.8, y = 3.8, size = 3, color = control2) + 
...
*manuscript*
note, 3132 rows removed bc they do not have cloacal temp
...
ggplot(CEWL_time_temp_subset) +
  ...
  # annotations for heating group
   ...
   annotate("text", x = 20.5, y = 40, label = "T[1]", parse = T,
           size = 5, family = "sans", color = heat) +
  
   #annotate(geom = "point", x = 38.3, y = 25.2, 
    #       size = 4, pch = 24, fill = heat) +
   annotate("text", x = 36.2, y = 65, label = "T[15]", parse = T,
           size = 5, family = "sans", color = heat) +
...
```{r}
ggsave(filename = "raw_CEWL_temp_dead.pdf",
       plot = CEWL_ctemp,
       path = "./Results_Figures",
       device = "pdf",
       dpi = 600,
