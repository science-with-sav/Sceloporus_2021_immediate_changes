...
# load data from live lizards
live_heat_CEWL <- read_rds("./Data/CEWL_by_temp_heating_tmt.csv")
...
heating_compare <- mean_CEWL_temp_dead %>%
  ...
  dplyr::select(treatment, 
                rounded_cal_c_temp, 
                heating_mean_CEWL_dead = mean_CEWL) %>%
  left_join(live_heat_CEWL, by = 'rounded_cal_c_temp') %>%
...
heating_compare %>%
  ...
  mutate(sem_diff = sd_diff/sqrt(nrow(heating_compare)))

# compare cooling tmt dead vs live
# load data from live lizards
live_cool_CEWL <- read_rds("./Data/CEWL_by_temp_cooling_tmt.csv")
cooling_compare <- mean_CEWL_temp_dead %>%
  ...
  left_join(live_cool_CEWL, by = 'rounded_cal_c_temp') %>%
  mutate(dead_minus_live_diff = cooling_mean_CEWL_dead - cooling_mm_CEWL)

cooling_compare %>%
  summarise(mean_diff = mean(dead_minus_live_diff, na.rm = T), 
            sd_diff = sd(dead_minus_live_diff, na.rm = T), 
            max_diff = max(dead_minus_live_diff, na.rm = T), 
            min_diff = min(dead_minus_live_diff, na.rm = T)) %>%
  mutate(sem_diff = sd_diff/sqrt(nrow(cooling_compare)))
```
